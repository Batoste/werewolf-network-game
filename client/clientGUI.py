        ...
        if self.sock and self.running:
            try:
                ...
                if msg_type in [
                    ...
                    MessageType.NIGHT_VOTE.value,
                    MessageType.START.value,
                    MessageType.RESTART.value,
                ]:
...
class WerewolfClient(QMainWindow):
    ...
    def handle_connected(self):
        ...
        if self.username and self.username not in [
            self.players_list_widget.item(i).text()
        ...
        if msg_type == "ROLE":
            ...
            if payload == "villagers_win":
                ...
                QMessageBox.information(
                    self,
                    "Game Over",
                    "Villagers have won! The werewolves were eliminated.",
                )
                ...
                QMessageBox.information(
                    self,
                    "Game Over",
                    "Werewolves have won! They devoured all the villagers.",
                )
                ...
                self.add_chat_message(
                    "STATE",
                    "☀️ Day breaks! Discuss and vote to eliminate a suspect.",
                    color,
                )
                ...
                self.add_chat_message(
                    "STATE",
                    "🌙 Night falls... Special roles act in the shadows.",
                    color,
                )
            ...
            try:
                ...
                self.add_chat_message(
                    "SEER", f"💫 You discovered that {name} is a {role}!", "#9c88ff"
                )
                # Show a more visible message
                QMessageBox.information(
                    self, "Seer's Vision", f"You discovered that {name} is a {role}!"
                )
            except Exception as e:
            ...
            if payload not in [
                ...
                self.add_chat_message(
                    "PLAYER", f"👋 {payload} joined the game!", "#3742fa"
                )

        elif msg_type == "ROLE_DISTRIBUTION":
            # Afficher la répartition des rôles
            self.add_chat_message(
                "INFO", f"📊 Role distribution for this game: {payload}", "#3742fa"
            )
            ...
            for i in range(self.players_list_widget.count()):
                if self.players_list_widget.item(
                    i
                ).text() == payload or self.players_list_widget.item(
                    i
                ).text().startswith(
                    payload + " "
                ):
    ...
    def update_buttons_visibility(self):
        ...
        self.night_vote_btn.setVisible(
            self.game_state == "night" and self.player_role == "werewolf" and is_alive
        )
    ...
    def create_game_info_panel(self, parent_layout):
        ...
        self.role_label.setStyleSheet(
            "font-weight: bold; color: #ff9f43; font-size: 14px;"
        )
        ...
        self.state_label.setStyleSheet(
            "font-weight: bold; color: #3742fa; font-size: 14px;"
        )
        ...
        self.players_list_widget.customContextMenuRequested.connect(
            self.show_player_context_menu
        )
        self.players_list_widget.setStyleSheet(
            "background-color: #2f3542; color: #f1f2f6; border-radius: 5px;"
        )

        # Instructions pour la liste des joueurs
        player_info_label = QLabel("Cliquez-droit sur un joueur pour agir")
        player_info_label.setStyleSheet(
            "font-style: italic; color: #a4b0be; font-size: 11px;"
        )
    ...
    def create_chat_panel(self, parent_layout):
        ...
        chat_tabs.setStyleSheet(
            "QTabBar::tab:selected {background-color: #3742fa; color: white; font-weight: bold;}"
        )
        ...
        self.message_input.setStyleSheet(
            "background-color: #2f3542; color: #f1f2f6; border-radius: 5px; padding: 5px;"
        )
    ...
    def create_command_panel(self, parent_layout):
        ...
        self.cmd_history.setStyleSheet(
            "background-color: #2f3542; color: #a4b0be; border-radius: 5px; font-family: 'Consolas', monospace;"
        )
        ...
        prompt_label.setStyleSheet(
            "color: #2ed573; font-weight: bold; font-size: 14px; font-family: 'Consolas', monospace;"
        )
        cmd_input_layout.addWidget(prompt_label)

        self.command_input = QLineEdit()
        self.command_input.setPlaceholderText(
            "/vote <player>, /nvote <player>, /start, /restart, /help"
        )
        self.command_input.setStyleSheet(
            "background-color: #2f3542; color: #f1f2f6; border-radius: 5px; padding: 5px; font-family: 'Consolas', monospace;"
        )
    ...
    def night_vote_for_player(self, player_name):
        ...
        if reply == QMessageBox.Yes:
            ...
            self.add_chat_message(
                "NIGHT VOTE", f"You chose to attack {player_name}", "#ff6b9d"
            )
    ...
    def witch_kill_player(self, player_name):
        self.network_worker.send_message(
            MessageType.NIGHT_VOTE.value, f"witch_kill:{player_name}"
        )
    ...
    def whisper_to_player(self, player_name):
        ...
        self.add_chat_message(
            "SYSTEM", "Private messages are not implemented yet", "#ff6b6b"
        )

    def show_vote_dialog(self):
        if not self.players_list_widget.count():
            QMessageBox.warning(
                self, "Unable to vote", "No players available for voting."
            )
            return
        ...
        target, ok = QInputDialog.getItem(
            self, "Vote", "Choose a player to eliminate:", players, 0, False
        )
        if ok and target:
    ...
    def show_night_action_dialog(self):
        ...
        if self.game_state != "night":
            QMessageBox.information(
                self, "Night action", "Special actions can only be performed at night."
            )
            return
    ...
    def show_night_vote_dialog(self):
        ...
        # Build the list of living players for night voting
        ...
        for i in range(self.players_list_widget.count()):
            ...
            if item:
                ...
                if "(mort)" not in player_name and player_name != self.username:
                    clean_name = player_name.split(" (")[
                        0
                    ]  # Ne garder que le nom sans les annotations
        ...
        target, ok = QInputDialog.getItem(
            self, "Night Vote", "Choose a victim for tonight:", players, 0, False
        )
        if ok and target:
    ...
    def witch_action(self, action, dialog):
        if action == "none":
            ...
            self.add_chat_message(
                "WITCH", "You used your healing potion to save the victim", "#ff6348"
            )
        dialog.accept()
    ...
    def witch_select_target(self, parent_dialog):
        ...
        for i in range(self.players_list_widget.count()):
            item = self.players_list_widget.item(i)
            if item and "(mort)" not in item.text():
                players.append(
                    item.text().split(" (")[0]
                )  # Keep only the name without the role

        # Check that the list is not empty
        if not players:
            QMessageBox.warning(
                self, "Action impossible", "No player available to poison."
            )
            return

        target, ok = QInputDialog.getItem(
            self, "Poison", "Choose a player to poison:", players, 0, False
        )
        if ok and target:
    ...
    def show_seer_dialog(self):
        ...
        if not players:
            QMessageBox.warning(
                self, "Action impossible", "No player available to inspect."
            )
            return

        target, ok = QInputDialog.getItem(
            self, "Seer Vision", "Choose a player to inspect:", players, 0, False
        )
        if ok and target:
    ...
    def show_hunter_dialog(self):
        ...
        if not players:
            QMessageBox.warning(
                self, "Action impossible", "No player available to shoot."
            )
            return

        target, ok = QInputDialog.getItem(
            self, "Hunter Shot", "Choose a player to shoot:", players, 0, False
        )
        if ok and target:
    ...
    def process_command(self):
        ...
        if command.startswith("/vote "):
            ...
            self.add_chat_message(
                "COMMANDE", f"Vote de nuit contre {target}", "#2ed573"
            )
        elif command == "/start":
            ...
            if len(parts) >= 3:
                ...
                self.add_chat_message(
                    "ERREUR",
                    "Format incorrect. Utilisez /whisper <joueur> <message>",
                    "#ff6b6b",
                )
        elif command == "/help":
            self.show_help()
            ...
            self.add_chat_message(
                "ERREUR",
                "Commande inconnue. Tapez /help pour voir les commandes.",
                "#ff6b6b",
            )
    ...
    def restart_game(self):
        reply = QMessageBox.question(
            self, "Redémarrer", "Voulez-vous vraiment redémarrer le jeu?"
        )
    ...
    def add_chat_message(self, msg_type, message, color="#ffffff"):
        ...
        self.chat_display.append(
            f'<span style="color: #a4b0be;">[{timestamp}]</span> <span style="color: {color}; font-weight: bold;">[{msg_type}]</span> {message}'
        )
    ...
    def add_to_command_history(self, command):
        ...
        self.cmd_history.append(
            f'<span style="color: #a4b0be;">[{timestamp}]</span> <span style="color: #2ed573;">{command}</span>'
        )
        self.cmd_history.verticalScrollBar().setValue(
            self.cmd_history.verticalScrollBar().maximum()
        )
    ...
    def update_buttons_visibility(self):
        ...
        self.night_vote_btn.setVisible(
            ...
            and is_alive
            and self.player_role in ["werewolf", "voyante", "sorcière", "chasseur"]
        )
        ...
        if self.player_role in role_icons:
            self.role_label.setText(
                f"{role_icons[self.player_role]} {self.player_role}"
            )
